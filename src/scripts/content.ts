...
function getMediaInfo() {
    ...
    return {
        ...
        volume: media.volume,
        media_title: (navigator.mediaSession?.metadata?.title) || document.title || "Unknown Track",
        media_artist: navigator.mediaSession?.metadata?.artist || "",
        media_album_name: navigator.mediaSession?.metadata?.album || "",
...
function triggerMediaCommand(command: "next" | "previous") {
    ...
    const searchTerms = command === "next" ? ["next", "skip forward"] : ["previous", "skip backward", "prev"];
...
// Initial setup
document.querySelectorAll('video, audio').forEach(m => setupListeners(m as HTMLMediaElement));
sendUpdate();
